...
/// - 3.b In `init_module`, we store the resource account's signer capability within `ModuleData` for later usage. // TODO: check: what is the 'signer capability'?
/// - 3.c In `mint_event_ticket`, we create a resource signer by calling `account::create_signer_with_capability(&module_data.signer_cap)` to programmatically sign for `token::mint_token()` and `token::direct_transfer()` functions. // TODO: check: what is the 'resource signer'?
...
module mint_nft::create_nft_with_resource_account {
    ...
    fun init_module(resource_signer: &signer) {
        ...
        let token_name = string::utf8(b"LMProfileV1: 0x000001's Profile");
        let token_uri = string::utf8(b"ipfs://QmcVg5vXZdxobFuz44YRCV2cubFRKJzBjvedfmLB9UqsWu");
