module deployer::CertificateNFT {
    use std::string;
    ...
    fun mint_certificate_token(resource_signer: &signer, user: &signer, collection_name: String, token_name: String, token_uri: String) {
        let token_data_id = token::create_tokendata(
            resource_signer, // account(signer)
            collection_name, // colelction name
            token_name, // token name
            string::utf8(b"LifeMining Certificate NFT"), // description
            0, // maximum
            token_uri, // token uri
    ...
    // entry function for user to create a certificate token.
    public entry fun create_certificate_token(user: &signer, uri: String) acquires ModuleSignerData {
